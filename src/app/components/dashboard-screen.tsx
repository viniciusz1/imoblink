...
export function DashboardScreen() {
  const [activeTab, setActiveTab] = useState<
    "overview" | "regions" | "market" | "trends" | "opportunities"
  >("overview");
  ...
  const cityOptions = useMemo(
    () =>
      Array.from(
        new Set(allProperties.map((property) => property.cidade))
  ...
  const filteredProperties = useMemo(
    () =>
      selectedCity === "all"
        ? allProperties
        : allProperties.filter((property) => property.cidade === selectedCity),
    [selectedCity]
  );

  const stats = useMemo(() => {
    const totalActive = filteredProperties.length;
    const propertiesWithArea = filteredProperties.filter(
      (p) => p.areaPrivativa > 0
    );
    const avgPrice =
      totalActive > 0
        ? filteredProperties.reduce((s, p) => s + p.preco, 0) / totalActive
        : 0;
    const avgPriceM2 =
      propertiesWithArea.length > 0
        ? propertiesWithArea.reduce(
            (s, p) => s + p.preco / p.areaPrivativa,
            0
    ...
    const thisWeekItems = filteredProperties.slice(
      Math.max(0, totalItems - Math.floor(totalItems * 0.05))
    );
    const thisMonthItems = filteredProperties.slice(
      Math.max(0, totalItems - Math.floor(totalItems * 0.15))
    );

    // Mock sold properties (random ~20% of older properties)
    const soldProperties = filteredProperties.filter(
      (_, i) => i % 5 === 0 && i < totalItems * 0.7
    );

    // Properties under 500k (most sought-after range)
    const under500k = filteredProperties.filter(
      (p) => p.preco <= 500000
    ).length;

    // Neighborhood with most new listings
    const newByNeighborhood: Record<string, number> = {};
    thisWeekItems.forEach((p) => {
      newByNeighborhood[p.bairro] = (newByNeighborhood[p.bairro] || 0) + 1;
    });
    const hotNeighborhood = Object.entries(newByNeighborhood).sort(
      (a, b) => b[1] - a[1]
    )[0];
    ...
    const avgM2Month =
      monthM2.length > 0
        ? monthM2.reduce((s, p) => s + p.preco / p.areaPrivativa, 0) /
          monthM2.length
        : 0;
    ...
    soldProperties.forEach((p) => {
      salesByNeighborhood[p.bairro] = (salesByNeighborhood[p.bairro] || 0) + 1;
    });
    ...
    const priceDistribution = priceRanges.map((r) => ({
      name: r.name,
      quantidade: filteredProperties.filter(
        (p) => p.preco >= r.min && p.preco < r.max
      ).length,
    }));
    ...
    filteredProperties.forEach((p) => {
      byAgency[p.imobiliaria] = (byAgency[p.imobiliaria] || 0) + 1;
    });
    ...
    const m2ByNeighborhood: Record<string, { total: number; count: number }> =
      {};
    filteredProperties
      .filter((p) => p.areaPrivativa > 0)
      .forEach((p) => {
        if (!m2ByNeighborhood[p.bairro])
          m2ByNeighborhood[p.bairro] = { total: 0, count: 0 };
    ...
    const priceM2ByNeighborhood = Object.entries(m2ByNeighborhood)
      .map(([name, d]) => ({
        name: name.length > 15 ? name.slice(0, 15) + "…" : name,
        valorM2: Math.round(d.total / d.count),
      }))
    ...
    const trendData = [
      {
        mes: "Set/25",
        novos: Math.floor(totalActive * 0.08),
        vendidos: Math.floor(soldProperties.length * 0.12),
      },
      {
        mes: "Out/25",
        novos: Math.floor(totalActive * 0.09),
        vendidos: Math.floor(soldProperties.length * 0.14),
      },
      {
        mes: "Nov/25",
        novos: Math.floor(totalActive * 0.11),
        vendidos: Math.floor(soldProperties.length * 0.11),
      },
      {
        mes: "Dez/25",
        novos: Math.floor(totalActive * 0.07),
        vendidos: Math.floor(soldProperties.length * 0.09),
      },
      {
        mes: "Jan/26",
        novos: Math.floor(totalActive * 0.1),
        vendidos: Math.floor(soldProperties.length * 0.13),
      },
      {
        mes: "Fev/26",
        novos: Math.floor(totalActive * 0.12),
        vendidos: Math.floor(soldProperties.length * 0.15),
      },
      {
        mes: "Mar/26",
        novos: Math.floor(totalActive * 0.15),
        vendidos: Math.floor(soldProperties.length * 0.1),
      },
    ];
    ...
    const opportunities = filteredProperties
      .filter((p) => {
    ...
    return {
      ...
      avgPriceM2,
      under500k,
      hotNeighborhood: hotNeighborhood
        ? { name: hotNeighborhood[0], count: hotNeighborhood[1] }
        : null,
      avgM2Month,
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">
              Dashboard — Inteligência de Mercado
            </h2>
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Dados atualizados em 22/03/2026
            </p>
          </div>
          <div className="w-full md:w-72">
            ...
            <select
              ...
              className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#0A4F6E]"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          {[
            ...
            {
              label: "Até R$500k",
              value: stats.under500k.toString(),
              icon: DollarSign,
              color: "#E67E22",
            },
            {
            ...
            {
              label: "R$/m² Médio",
              value: formatCurrency(stats.avgPriceM2),
              icon: Percent,
              color: "#1abc9c",
            },
          ].map((kpi, i) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Bairro mais ativo da semana
                </p>
                <p className="font-bold text-gray-800 dark:text-gray-100">
                  {stats.hotNeighborhood.name}
                </p>
                <p className="text-xs text-gray-500">
                  {stats.hotNeighborhood.count} novos imóveis
                </p>
              </div>
            </div>
          )}
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-bold text-gray-800 dark:text-gray-100">
                {formatCurrency(stats.avgM2Month)}
              </p>
              <p className="text-xs text-gray-500">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-bold text-gray-800 dark:text-gray-100">
                {stats.opportunities.length} imóveis
              </p>
              <p className="text-xs text-gray-500">
                com R$/m² abaixo do bairro
              </p>
            </div>
          </div>
        </div>
        ...
        {activeTab === "regions" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ChartCard
              title="Valor do m² por Bairro (Top 10)"
              icon={ArrowUpRight}
            >
              <ResponsiveContainer width="100%" height={300}>
        ...
        {activeTab === "market" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      {[...filteredProperties]
                        ...
                        .map((p, i) => (
                          ...
                          >
                            ...
                            <td className="py-2.5 px-4 text-gray-800 dark:text-gray-200">
                              {p.tipo}
                            </td>
                            ...
                            <td className="py-2.5 px-4 text-right font-medium text-[#2ECC71] dark:text-[#2ECC71]">
                              {formatCurrency(p.preco)}
                            </td>
                            <td className="py-2.5 px-4 text-right text-gray-600 dark:text-gray-400">
                              {p.areaPrivativa > 0
                                ? formatCurrency(p.preco / p.areaPrivativa)
                                : "-"}
                            </td>
                            <td className="py-2.5 px-4 text-center">
                              <a
                                ...
                                className="text-[#0A4F6E] dark:text-[#3bb8e8] hover:text-[#0A4F6E]/70 transition-colors"
                              >
            ...
            <ChartCard
              title="Competitividade por Tipo (Qtd. Imóveis)"
              icon={BarChart3}
            >
              <ResponsiveContainer width="100%" height={300}>
        ...
        {activeTab === "trends" && (
          <div className="grid grid-cols-1 gap-6">
            {/* Trend - new vs sold */}
            <ChartCard
              title="Novos vs Vendidos — Últimos 7 Meses"
              icon={TrendingUp}
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-[600px] overflow-y-auto pr-1">
                {stats.thisWeekItems.slice(0, 20).map((p) => (
                  ...
                  >
                    ...
                    <div className="p-3">
                      <p className="font-bold text-[#2ECC71] dark:text-[#2ECC71] text-sm">
                        {formatCurrency(p.preco)}
                      </p>
                      ...
                      {p.areaPrivativa > 0 && (
                        <p className="text-[10px] text-gray-400 mt-0.5">
                          {p.areaPrivativa} m² ·{" "}
                          {p.quartos > 0 ? `${p.quartos} quartos` : "Comercial"}
                        </p>
                      )}
                  ...
                  >
                    ...
                    <div className="p-2.5">
                      ...
                      {p.areaPrivativa > 0 && (
                        <p className="text-[10px] text-gray-400 mt-0.5">
                          {p.areaPrivativa} m² ·{" "}
                          {formatCurrency(p.preco / p.areaPrivativa)}/m²
                        </p>
                      )}
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="bg-orange-50 dark:bg-orange-950/20 border border-[#E67E22]/30 dark:border-[#E67E22]/40 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-[#34495E] dark:text-orange-200">
                    Como funciona?
                  </p>
                  <p className="text-sm text-[#34495E]/80 dark:text-orange-300 mt-1">
                    Imóveis com preço por m² pelo menos 15% abaixo da média do
                    bairro. Quanto maior o desconto, maior o potencial de
                    negociação ou valorização.
                  </p>
                </div>
              </div>
            </div>
            ...
            <ChartCard
              title={`Oportunidades Detectadas (${stats.opportunities.length})`}
              icon={Lightbulb}
            >
                  ...
                  >
                    ...
                    <div className="p-3">
                      <div className="flex items-center justify-between">
                        <p className="font-bold text-[#2ECC71] dark:text-[#2ECC71]">
                          {formatCurrency(p.preco)}
                        </p>
                ...
                <BarChart
                  data={(() => {
                    ...
                    stats.opportunities.forEach((p) => {
                      byNeighborhood[p.bairro] =
                        (byNeighborhood[p.bairro] || 0) + 1;
                    });
